...
{
  /* On Protea the visible cursor is drawn by the hardware text cursor
   * (see the main loop syncing terminal_get_cursor -> display_set_cursor),
   * so the engine does not invert the cell here. */
  (void)show;
}

void terminal_get_cursor(int *col, int *row, bool *visible)
{
  if( col )     *col = cursor_col;
  if( row )     *row = cursor_row;
  if( visible ) *visible = cursor_shown;
}
